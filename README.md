...
Unit tests are located under `central-core-hub/tests/unit/` and can be run locally with:
...
Notes:
- Tests dynamically load `central-core-hub/mqtt_client.py` by file path to avoid import issues when the repository
  directory contains hyphens.
- Running the full suite currently produces a benign urllib3/OpenSSL warning on macOS (LibreSSL); tests still pass.
