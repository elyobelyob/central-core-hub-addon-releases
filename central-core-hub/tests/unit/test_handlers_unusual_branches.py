...
def test_ack_publish_raises_but_processing_continues(monkeypatch):
    ...
    handlers.handle_message(c, msg, msg.payload.decode("utf-8"), fetch_sensors, None, None, None)
...
def test_set_requests_post_raises_results_failed(monkeypatch):
    ...
    requests = type("R", (), {"post": staticmethod(bad_post), "get": staticmethod(lambda *a, **k: FakeResp())})
