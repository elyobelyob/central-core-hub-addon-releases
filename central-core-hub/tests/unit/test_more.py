...
def test_publish_telemetry_and_vault(monkeypatch):
    ...
    monkeypatch.setattr(mod, 'build_telemetry', lambda cid: json.dumps({'client_id': cid}))
    monkeypatch.setattr(mod, 'build_vault_payload', lambda raw: json.dumps({'schema_version': 2}))

    options = {'client_id': 'unit-123', 'mqtt_host': 'x', 'vault_topic': 'vault/unit-123/telemetry'}
