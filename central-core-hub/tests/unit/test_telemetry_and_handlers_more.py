...
def test_build_telemetry_with_injected_helpers():
    ...
    payload = tele.build_telemetry('cid-x', get_cpu_percent=lambda: 3.3, uptime_fn=lambda: 111, loadavg_fn=lambda: [0.1, 0.2, 0.3], mem_info_fn=lambda: (1024, 512), disk_info_fn=lambda: (2048, 1024))
...
def test_build_vault_payload_extracts_metrics():
    tele = _load_module('telemetry.py')
    raw = json.dumps({'client_id': 'c1', 'timestamp': 't', 'hostname': 'h', 'ip': '1.2.3.4', 'cpu_count': 1, 'cpu_percent': 5, 'uptime': 10, 'mem_total_kb': 100, 'mem_free_kb': 50, 'disk_total_kb': 200, 'disk_free_kb': 100})
...
def test_handlers_poll_with_disabled_and_names(monkeypatch):
    ...
    sensors = [
        {'entity_id': 'sensor.a', 'state': 'on', 'name': 'A', 'attributes': {'friendly_name': 'Friendly A', 'disabled_by': None}},
        {'entity_id': 'sensor.b', 'state': '0', 'name': 'B', 'attributes': {'friendly_name': 'Friendly B', 'disabled_by': 'user'}}
    ]
...
def test_handlers_set_no_ha_config_causes_failed(monkeypatch):
    ...
    msg = types.SimpleNamespace(topic='hubs/hub2/cmd/sensors/set', payload=payload.encode('utf-8'))
